...
public final class Runner {
    ...
    public static void main(String[] args) {
        ...
        if (isRunning()) {
            ...
            JOptionPane.showMessageDialog(null, "Another instance of 'GPW Monitor' "
                    + "is already running - only one instance can be up.", "GPW Monitor",
    ...
    private static GpwMonitor initialize(CountDownLatch latch) {
        ...
        monitor.scheduleAtFixedRate(new RecommendationReaderTask(myWallet, recListeners, objectListeners),
                TEN_SECONDS, HOUR, TimeUnit.SECONDS);
    ...
    private static boolean isRunning() {
        try {
            RandomAccessFile lock = new RandomAccessFile(".lock", "rw");
            return (lock.getChannel().tryLock() == null);
        } catch (IOException e) {
