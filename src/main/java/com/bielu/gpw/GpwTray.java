...
public class GpwTray {
    ...
    private final Closable closableObj;
    ...
    protected GpwTray(Closable closable, CountDownLatch latch) {
        this.latch = latch;
    ...
    private PopupMenu setupPopupMenu() {
        ...
        for (MenuItemInfo info : Configuration.getInstance().getMenuItemList()) {
            ...
            try {
                ...
                LOG.error(String.format("Class [%s] does not exist in the classpath "
                        + "or could not be instantiated", info.getClassName()), e);
            }
        }
        return popup;
    }
