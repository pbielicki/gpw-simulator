...
public class TrayListener implements ChangeListener<Wallet> {
    ...
    public void stateChanged(Wallet current) {
        ...
        if (Util.checkPoint(current, checkPoint) || getCheckPoint() == current) {
            setCheckPoint(current);
    ...
    private void appendDiff(Wallet original, Wallet current, StringBuilder sb) {
        for (int i = 0; i < current.size(); i++) {
            ...
            if (cur.hasError()) {
                ...
                sb.append(String.format(getFormat(cur.getName()), 
                        cur.getName(), 
                        cur.getQuote(), 
                        Util.percentagePlus(wal, cur)));
            }
        }
        sb.append(String.format("Total:\t\t%.2f\t(%s)", current.getWalletValue(), Util.percentagePlus(original, current)));
    }
