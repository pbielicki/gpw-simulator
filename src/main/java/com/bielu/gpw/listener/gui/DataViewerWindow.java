...
public class DataViewerWindow extends JFrame {
    ...
    private volatile Wallet currentWallet;
    ...
    protected synchronized Wallet getCurrentWallet() {
        return currentWallet;
    }

    protected synchronized Wallet getCheckPoint() {
        return checkPoint;
    }
    ...
    private class WalletTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            } else if (rowIndex == myWallet.size()) {
                return "-----";
            } else {
        ...
        private String getShareInfo(int rowIndex, int columnIndex) {
            ...
            if (currentWallet != null) {
                ShareInfo currentShare = currentWallet.getShareInfo(rowIndex);
                switch (columnIndex) {
                    case CURRENT_QUOTE:
                    ...
                    case CURRENT_PROFIT:
                        return String.format("%s (%s)",
                                Util.diff(share, currentShare),
                                Util.percentagePlus(share, currentShare));
                    case YEARLY_RATE:
                        return String.format("%s",
                                Util.yearlyRate(share, currentShare));
                    default:
                }
            }
            return "";
        ...
        private String getWalletInfo(int columnIndex) {
            ...
            if (currentWallet != null) {
                switch (columnIndex) {
                    case CURRENT_VALUE:
                        return String.format("%.2f", currentWallet.getWalletValue());
                    case CURRENT_PROFIT:
                        return String.format("%s (%s)",
                                Util.diff(myWallet, currentWallet),
                                Util.percentagePlus(myWallet, currentWallet));
                    default:
                }
            }
            return "";
